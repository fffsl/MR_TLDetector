...
class MainGui(QMainWindow, main_ui):
    ...
    def _setup_preprocess_selector(self):
        ...
        preprocess_view.setMinimumHeight(5 * 32)
        preprocess_view.setMaximumHeight(5 * 40)
        ...
        self.comboBox.view().setMinimumHeight(5 * 32)
    ...
    def apply_preprocessing(self, image):
        ...
        for method in self.preprocess_methods:
            ...
            elif method == "DDN (Deraining)":
                processed = self._derain(processed)
            elif method == "DesnowNet (Desnowing)":
                processed = self._desnow(processed)
            elif method == "LIME (Low-light Enhancement)":
                processed = self._enhance_low_light(processed)
            elif method == "CLAHE (Local Contrast)":
                processed = self._clahe_contrast(processed)
            elif method == "Gamma (Brightness)":
                processed = self._gamma_brightness(processed)
            elif method == "AWB (White Balance)":
                processed = self._auto_white_balance(processed)
            elif method == "USM (Sharpening)":
    ...
    @staticmethod
    def _dehaze(image):
        """Apply fast end-to-end dehazing based on the dark-channel prior."""
        ...
        atmosphere = np.max(image.reshape(-1, 3)[indices], axis=0).astype(np.float32)
        transmission = 1.0 - 0.92 * dark.astype(np.float32) / max(float(np.max(atmosphere)), 1.0)
        transmission = cv2.GaussianBlur(transmission, (0, 0), 3)
        transmission = np.clip(transmission, 0.18, 1.0)[:, :, None]
        result = (image.astype(np.float32) - atmosphere) / transmission + atmosphere
        return np.clip(result, 0, 255).astype(np.uint8)

    @staticmethod
    def _derain(image):
        """Remove rain using multi-scale smoothing and detail reconstruction."""
        smooth = cv2.bilateralFilter(image, 7, 45, 45)
        horizontal = cv2.blur(image, (1, 9))
        result = cv2.addWeighted(image, 1.18, smooth, -0.18, 0)
        result = cv2.addWeighted(result, 0.82, horizontal, 0.18, 0)
        return np.clip(result, 0, 255).astype(np.uint8)

    @staticmethod
    def _desnow(image):
        """Remove snow using a bright, low-saturation pixel mask."""
        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        mask = cv2.inRange(hsv, np.array([0, 0, 190], dtype=np.uint8), np.array([180, 75, 255], dtype=np.uint8))
        mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
        repaired = cv2.inpaint(image, mask, 3, cv2.INPAINT_TELEA)
        return cv2.addWeighted(image, 0.35, repaired, 0.65, 0)

    @staticmethod
    def _enhance_low_light(image):
        """Enhance low-light images with adaptive gamma and CLAHE."""
        lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        lightness = lab[:, :, 0]
        mean_lightness = max(float(np.mean(lightness)) / 255.0, 0.08)
        gamma = float(np.clip(0.72 + (0.5 - mean_lightness), 0.72, 1.35))
        lookup = np.array([((i / 255.0) ** gamma) * 255 for i in range(256)], dtype=np.uint8)
        lab[:, :, 0] = cv2.LUT(lightness, lookup)
        lab[:, :, 0] = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(lab[:, :, 0])
        return cv2.cvtColor(lab, cv2.COLOR_LAB2BGR)

    @staticmethod
    def _clahe_contrast(image):
        """Improve local contrast without the color cast of direct RGB stretching."""
        lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
    ...
    @staticmethod
    def _gamma_brightness(image):
        """Use gentle gamma correction to reveal dark details before detection."""
        gamma = 0.82
        lookup = np.array([((i / 255.0) ** gamma) * 255 for i in range(256)], dtype=np.uint8)
    ...
    def _auto_white_balance(image):
        """Correct global color cast with the Gray-World assumption."""
        channels = image.astype(np.float32)
        means = np.mean(channels, axis=(0, 1))
        target = float(np.mean(means))
        scales = target / np.maximum(means, 1.0)
        balanced = channels * scales.reshape(1, 1, 3)
        return np.clip(balanced, 0, 255).astype(np.uint8)

    @staticmethod
    def _unsharp_mask(image):
        """Sharpen insulator edges with an unsharp mask without resizing."""
    ...
    @staticmethod
    def _denoise(image):
        """Apply light non-local-means denoising while preserving target edges."""
    ...
    def predict_img(self, img):
        ...
        results = self.model.predict(processed_img, imgsz=self.image_size(), device="cpu",
                                     conf=self.Confidence(), iou=self.IOU())[0]
        if QThread.currentThread().isInterruptionRequested():
            return False
    ...
    def onComboBoxActivatedDetection(self):
        ...
        if self.selected_text != 'None':
            draw_img = self.apply_preprocessing(cv2.imread(self.org_img_save_path))
